#based on function error.bars in R book, page 168
errorBars <- function(means,errors,title="",ylabel="", xlabel="",ylimitation=c(min(means)-min(means)/10,max(means)+max(means/10)),color="grey",spaceBesidesBar=0.2)
{
  ...
  {
    #err <- sd(phoneLowBegin0)/sqrt(length(phoneLowBegin0))
...
### #Functions from website: 	http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_%28ggplot2%29/ 
### Check that website for futher instructions
