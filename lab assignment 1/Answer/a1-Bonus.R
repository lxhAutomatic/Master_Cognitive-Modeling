...
steeringTimeOptions <- c(1,2,4,6,8,10,12)
...
singleTaskKeyPressTimes <- c(258,258,258,258,258,258,258,258,258,258,258)
...
runAllComplexStrategies <- function(nrSimulations,phoneNumber)
{
  ...
  for(row in 1:nrow(combinationList))
  {
    ...
    comb <- split(combinationList[row,], seq(1))
    #print(comb)
    
    strategy <- which(combinationList[row,] %in% c(1))
    ...
    #print(strategy)
    # avgNrDigitsPerTime <- 11/(length(strategy)+1)
    currVal <- 0
    nrDigitsPerTime <- length(strategy)
    ...
    for (steerTimes in locSteerTimeOptions)
    {
      for (i in 1:nrSimulations)
      {
        ...
        strats <- c(strats,rep(row,nrow(locTab)))
  ...
  agrResultsMeanDriftNatBreak <- agrResultsMeanDrift %>%
    filter(strats == 17,)
  
  # make a plot that visualizes all the strategies: note that trial time is divided by 1000 to get the time in seconds
  #agrResultsMeanDrift <- with(agrResultsMeanDrift,agrResultsMeanDrift[TrialTime/1000,abs(dev)])
  #plot(dataPlot,pch=21,bg="dark grey",col="dark grey",log="x",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)")
  ...
  data$typingErrorMadeOnTrial
  
  noErrorMade = data[data$typingErrorMadeOnTrial == 0,]
  
  dualSteerFocus = noErrorMade[noErrorMade$partOfExperiment == "dualSteerFocus",]
  dualDialFocus = noErrorMade[noErrorMade$partOfExperiment == "dualDialFocus",]
  
  dial_mean_pp_pk = aggregate(abs(dualDialFocus$lanePosition), by=list(phoneNr = dualDialFocus$phoneNrLengthAfterKeyPress, pp = dualDialFocus$pp), FUN=mean)
  dial_mean_pk = aggregate(dial_mean_pp_pk$x, by=list(nr_length = dial_mean_pp_pk$phoneNr), FUN = mean)
  dial_mean_pp_pk <- dial_mean_pp_pk %>%
    filter(phoneNr == 12,)
  dial_mean_pk <- dial_mean_pk %>%
    filter(nr_length == 12,)
  
  dualDialFocusFiltered <- dualDialFocus %>%
    filter(phoneNrLengthAfterKeyPress == 12,)
  dualDialFocusFiltered$lanePosition <- abs(dualDialFocusFiltered$lanePosition)
  
  
  steer_mean_pp_pk = aggregate(abs(dualSteerFocus$lanePosition), by=list(phoneNr = dualSteerFocus$phoneNrLengthAfterKeyPress, pp = dualSteerFocus$pp), FUN=mean)
  steer_mean_pk = aggregate(steer_mean_pp_pk$x, by=list(nr_length = steer_mean_pp_pk$phoneNr), FUN = mean)
  steer_mean_pp_pk <- steer_mean_pp_pk %>%
    filter(phoneNr == 12,)
  steer_mean_pk <- steer_mean_pk %>%
    filter(nr_length == 12,)
  print(steer_mean_pp_pk)
  print(steer_mean_pk)
  
  dualSteerFocusFiltered <- dualSteerFocus %>%
    filter(phoneNrLengthAfterKeyPress == 12,)
  dualSteerFocusFiltered$lanePosition <- abs(dualSteerFocusFiltered$lanePosition)
  
  print(ci(dualSteerFocusFiltered$lanePosition, confidence=0.95))
  print(ci(dualDialFocusFiltered$lanePosition, confidence=0.95))
  
  low <- cbind(ci(dualSteerFocusFiltered$lanePosition, confidence=0.95)[2], ci(dualDialFocusFiltered$lanePosition, confidence=0.95)[2])
  #low <- cbind(steer_mean_pk$x, dial_mean_pk$x) - std(cbind(steer_mean_pk$x, dial_mean_pk$x))
  up <- cbind(ci(dualSteerFocusFiltered$lanePosition, confidence=0.95)[3], ci(dualDialFocusFiltered$lanePosition, confidence=0.95)[3])
  #up <- cbind(steer_mean_pk$x, dial_mean_pk$x) + std(cbind(steer_mean_pk$x, dial_mean_pk$x))
  
  
  
  dialDialTime_mean_pp <- aggregate(dualDialFocus$timeRelativeToTrialStart, by=list(phoneNr = dualDialFocus$phoneNrLengthAfterKeyPress,pp = dualDialFocus$pp),FUN=mean)
  dialDialTime_mean_pk <- aggregate((dialDialTime_mean_pp$x)/1000, by=list(nr_length = dialDialTime_mean_pp$phoneNr), FUN = mean)
  steerDialTime_mean_pp <- aggregate(dualSteerFocus$timeRelativeToTrialStart, by=list(phoneNr = dualSteerFocus$phoneNrLengthAfterKeyPress,pp = dualSteerFocus$pp),FUN=mean)
  steerDialTime_mean_pk <- aggregate((steerDialTime_mean_pp$x)/1000, by=list(nr_length = steerDialTime_mean_pp$phoneNr), FUN = mean)
  
  dualSteerFocusFiltered$timeRelativeToTrialStart <- dualSteerFocusFiltered$timeRelativeToTrialStart/1000
  dualDialFocusFiltered$timeRelativeToTrialStart <- dualDialFocusFiltered$timeRelativeToTrialStart/1000
  
  print(ci(dualSteerFocusFiltered$timeRelativeToTrialStart, confidence=0.95))
  print(ci(dualDialFocusFiltered$timeRelativeToTrialStart, confidence=0.95))
  
  dialDialTime_mean_pp <- dialDialTime_mean_pp %>%
    filter(phoneNr == 12,)
  steerDialTime_mean_pk <- steerDialTime_mean_pk %>%
    filter(nr_length == 12,)
  dialDialTime_mean_pk <- dialDialTime_mean_pk %>%
    filter(nr_length == 12,)
  steerDialTime_mean_pp <- steerDialTime_mean_pp %>%
    filter(phoneNr == 12,)
  
  low_h <- cbind(ci(dualSteerFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[2], ci(dualDialFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[2])
  #low_h <- cbind(steerDialTime_mean_pk$x, dialDialTime_mean_pk$x) - std(cbind(steerDialTime_mean_pk$x, dialDialTime_mean_pk$x))
  up_h <- cbind(ci(dualSteerFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[3], ci(dualDialFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[3])
  #up_h <- cbind(steerDialTime_mean_pk$x, dialDialTime_mean_pk$x) + std(cbind(steerDialTime_mean_pk$x, dialDialTime_mean_pk$x))
  
  #matplot(cbind(steerDialTime_mean_pk$x,dialDialTime_mean_pk$x), cbind(steer_mean_pk$x, dial_mean_pk$x),pch = 1:2, type = "o",col=c("red","green"),lty=c(1,1), ylim=c(0.25,0.85), xlab="Dialing Time (Sec)", ylab="Lateral Deviation(m)")
  #legend("topleft", inset=.05, legend=c("Dial Focus","Steer Focus"), pch=c(2,1), col=c("green","red"), horiz=FALSE)
  
  
  with(agrResultsMeanDrift,plot(TrialTime/1000,abs(dev),xlim=range(agrResultsMeanDrift$TrialTime/1000,agrResultsMeanDriftNatBreak$TrialTime/1000,low_h),pch=21,bg="dark grey",col="dark grey",log="x",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)"))
  with(agrResultsMeanDriftNatBreak,points(TrialTime/1000,abs(dev),pch=4,bg="black",col="black",log="x",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)"))
  
  
  xx <- cbind(ci(dualSteerFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[1],ci(dualDialFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[1])
  repcols <- rep(c("red","green"), each = nrow(cbind(steer_mean_pk$x, dial_mean_pk$x)))
  arrows(xx, low, xx, up, col = repcols, log="x",angle = 90, length = 0.03, code = 3)
  
  yy <- cbind(ci(dualSteerFocusFiltered$lanePosition, confidence=0.95)[1],ci(dualDialFocusFiltered$lanePosition, confidence=0.95)[1])
  repcols_h <- rep(c("red","green"), each = nrow(cbind(steerDialTime_mean_pk$x,dialDialTime_mean_pk$x)))
  arrows(low_h, yy, up_h, yy, col = repcols, log="x", angle = 90, length = 0.05, code = 3)
  
  points(x = ci(dualSteerFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[1],
         y = ci(dualSteerFocusFiltered$lanePosition, confidence=0.95)[1],
         pch = 23,
         col = "red")
  points(x = ci(dualDialFocusFiltered$timeRelativeToTrialStart, confidence=0.95)[1],
         y = ci(dualDialFocusFiltered$lanePosition, confidence=0.95)[1],
         pch = 22,
         col = "green")
  
  legend("topright", legend = c("Simulation Data", "Natural Interleaving", "Steering Focus", "Dialing Focus"), bty = "o",
         lwd = 2, cex=0.8, col = c("dark grey", "black", "red","green"), lty = c(NA, NA, NA, NA), pch = c(16,4,23,22))
...
runAllComplexStrategies(50,"07854325698")
