...
expected.success <- function(threshold, scale.points, densityf, cumulativef) {
    ...
    ifelse(threshold>min(scale.points), 
           sum(sapply(scale.points[scale.points<threshold], function(x) {densityf(x) * densityf(x)})), 
           0) + 
...
probability.threshold <- function(threshold, scale.points, lambda, coverage.parameter, densityf, cumulativef, denom = c()) {
    if(length(denom) == 0){
        denom = sapply(scale.points, function(t) exp(lambda*utility(t, scale.points, coverage.parameter, densityf, cumulativef)))
    }
    exp(lambda * utility(threshold, scale.points, coverage.parameter, densityf, cumulativef))/sum(denom)
}

use.adjective <- function(degree, scale.points, lambda, coverage.parameter, densityf, cumulativef, denom = c()) {
    if(length(denom) == 0){
        denom = sapply(scale.points, function(t) exp(lambda*utility(t, scale.points, coverage.parameter, densityf, cumulativef)))
    }
    ifelse(degree >= min(scale.points),
           sum(sapply(scale.points[scale.points <= degree], function(x) {probability.threshold(x, scale.points, lambda, coverage.parameter, densityf, cumulativef, denom)})),
           0)
}
...
gaussian.densityf = function(x){dnorm(x, 6, 2)}
gaussian.cummulativef = function(x){pnorm(x, 6, 2)}
...
likelihood <- function(param1) {
    ...
    denom = calc.denom(adj.points, lambda = param1[2], coverage.parameter = param1[1], gaussian.densityf, gaussian.cummulativef)
    
    for (i in 1:14) {
        collect <- collect + dnorm(data.gaus.big$percentage[i], mean=use.adjective(data.gaus.big$percentage[i], adj.points, lambda = param1[2], coverage.parameter = param1[1], gaussian.densityf, gaussian.cummulativef, denom), sd=0.1, log=TRUE)
    }
    return(collect)
}
...
mean=use.adjective(data.gaus.big$Stimulus[i], adj.points, lambda = param1[2], coverage.parameter = param1[1], gaussian.densityf, gaussian.cummulativef, denom), sd=0.1, log=TRUE

#Daan
#iqs <- sapply(1:30, function(x) { expected.success(x, scale.points, function(x) { dgamma(scale.points, shape=2, rate=1) }, function(x) { pgamma(scale.points, shape=2, rate=1) }) })
...
get_dist <- function(n, v) {
    if(n == 1) {
        if(v == 1) {
            return('gaussian')
        }
        if(v == 2) {
            return(function(x){dnorm(x, 6, 2)})
        }
        if(v == 3){
            return(function(x){pnorm(x, 6, 2)})
        }
    } else if(n == 2) {
        if(v == 1) {
            return('left')
        ...
        if(v == 2) {
            return(function(x){dgamma(x, shape = 4, scale = 1.5)})
        }
        if(v == 3) {
            return(function(x){pgamma(x, shape = 4, scale = 1.5)})
        }
    } else {
        if(v == 1) {
            return('moved')
        ...
        if(v == 2) {
            return(function(x){dnorm(x, 9, 2)})
        }
        if(v == 3) {
            return(function(x){pnorm(x, 9, 2)})
        }
    }
}

for(adj in c('big', 'pointy', 'tall')) {
    
    data.adjective.curr <- subset(data.adjective, Adjective == adj)
    
    for(n in c(1:3)){
        
        #data.adjective.three.curr <- data.adjective.three[data.adjective.three$Distribution==get_dist(n,1),
        
        likelihood <- function(param1) {
            
            collect <- 0
            
            denom = calc.denom(scale.points, lambda = param1[2], coverage.parameter = param1[1], get_dist(n,2), get_dist(n,3))
            
            for (i in 1:14) {
                collect  <- collect + dnorm(data.adjective.curr$percentage[i], mean=use.adjective(data.adjective.curr$percentage[i], scale.points, lambda = param1[2], coverage.parameter = param1[1], get_dist(n,2), get_dist(n,3), denom), sd=0.1, log=TRUE)
