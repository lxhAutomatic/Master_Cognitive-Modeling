...
\author{}
\date{Deadline: 24.1., individual assignments: 28.1.}
...
There are four prior distributions for the adjectives. These are the labels and their description in Solt and Gotzner/Qing and Franke:\footnote{Note that Solt \& Gotzner and Qing \& Franke use the label left skew for what is normally called ``right skew'' in statistics and vice versa. Usually, ``left skew'' refers to the skew with a long tail on the left side, and ``right skew'' refers to the skew with a long tail on the right side.}
...
\begin{itemize}
    \item Check that chains converged.
    \item Explore the summary and plot of the variable out. This variable stores samples from posterior distributions of the parameters. If everything worked fine, out should carry posterior distributions of coverage parameter and lambda.
\end{itemize}
